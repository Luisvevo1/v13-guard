...
module.exports = async (oldGuild, newGuild) => {
    ...
    if (oldGuild.banner !== newGuild.banner) await newGuild.setBanner(oldGuild.bannerURL({ size: 4096 }));
      return client.channels.cache.find(x => x.name == "guard-log").send({embeds: [new MessageEmbed()
        .setColor("RANDOM")
        .setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından sunucudan sunucu güncellendi! Güncelleyen kişi banlandı ve sunucu eski haline getirildi. https://discord.gg/gjjHmhTFhY https://discord.gg/gjjHmhTFhY`)] })
}
...
async function cezaVer(BotClient, KullanıcıID, CezaTürü) {
  ...
  if (!MEMBER) return;
  if (CezaTürü == "ban") return MEMBER.ban({reason: "Luis Ananı Sikti."}).catch(console.error);
